...
class Topics:
  ...
  @staticmethod
  def retrieve_topics(user_input: str) -> list[str]:
      array_of_topics = []
  ...
  def get_matching_group(topics: list) -> int:
      Config.GROUP_1["count"] = 0
      Config.GROUP_2["count"] = 0
      Config.GROUP_3["count"] = 0
      Config.GROUP_4["count"] = 0
      
      for word in topics:
          
          if (word in Config.GROUP_1["subtopics"]):
              Config.GROUP_1["count"] += 1
          
          if (word in Config.GROUP_2["subtopics"]):
              Config.GROUP_2["count"] += 1
              
          if (word in Config.GROUP_3["subtopics"]):
              Config.GROUP_3["count"] += 1
              
          if (word in Config.GROUP_4["subtopics"]):
              Config.GROUP_4["count"] += 1

      group = max([Config.GROUP_1, Config.GROUP_2, Config.GROUP_3, Config.GROUP_4], key=itemgetter('count'))
